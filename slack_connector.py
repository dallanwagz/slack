...
def handle_request(request, path):
    ...
    try:
        ...
        state_dir = f"{APPS_STATE_PATH}/{app_id}"
...
class SlackConnector(phantom.BaseConnector):
        ...
        try:
            ...
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR, SLACK_ERROR_UNABLE_TO_PARSE_JSON_RESPONSE.format(error=self._get_error_message_from_exception(e))
                ),
                None,
            )
    ...
    def _make_rest_call(self, action_result, rest_url, verify, method=requests.get, headers={}, body={}):
        try:
            ...
            return RetVal(
                action_result.set_status(phantom.APP_ERROR, f"{SLACK_ERROR_REST_CALL_FAILED}. {self._get_error_message_from_exception(e)}"),
                None,
            )
        ...
        if "failed" in resp_json:
            return RetVal(
                action_result.set_status(
                    phantom.APP_ERROR, "{}. Message: {}".format(SLACK_ERROR_REST_CALL_FAILED, resp_json.get("message", "NA"))
                ),
                None,
            )
    ...
    def _make_slack_rest_call(self, action_result, endpoint, body, headers={}, files={}):
        ...
        try:
            response = requests.post(f"{self._base_url}{endpoint}", data=body, headers=headers, files=files, timeout=SLACK_DEFAULT_TIMEOUT)
        except Exception as e:
            return RetVal(
                action_result.set_status(phantom.APP_ERROR, f"{SLACK_ERROR_SERVER_CONNECTION}. {self._get_error_message_from_exception(e)}"),
                None,
            )
    ...
    def _create_channel(self, param):
        ...
        endpoint = f"{SLACK_BASE_URL}{SLACK_CHANNEL_CREATE_ENDPOINT}"
        ...
        if not resp_json.get("ok", True):
            ...
            if error_details:
                error_message = f"{SLACK_ERROR_CREATING_CHANNEL}: {error}\r\nDetails: {error_details}"
            else:
                error_message = f"{SLACK_ERROR_CREATING_CHANNEL}: {error}"
    ...
    def _get_user(self, param):
        ...
        if not ret_val:
            ...
            if message:
                error_message = f"{SLACK_ERROR_FETCHING_USER}: {message}"
            else:
    ...
    def _invite_users(self, param):
        ...
        params = {"users": users, "channel": param["channel_id"], "token": user_token}
        ...
        if not resp_json.get("ok", True):
            ...
            error_details = resp_json.get("detail", "")
            if error_details:
                error_message = f"{SLACK_ERROR_INVITING_CHANNEL}: {error}\r\nDetails: {error_details}"
            else:
                error_message = f"{SLACK_ERROR_INVITING_CHANNEL}: {error}"
            return action_result.set_status(phantom.APP_ERROR, error_message)
    ...
    def _send_message(self, param):
        ...
        if not ret_val:
            message = action_result.get_message()
            if message:
                error_message = f"{SLACK_ERROR_SENDING_MESSAGE}: {message}"
            else:
    ...
    def _add_reaction(self, param):
        ...
        if not ret_val:
            message = action_result.get_message()
            if message:
                error_message = f"{SLACK_ERROR_ADDING_REACTION}: {message}"
            else:
    ...
    def _get_file_size(self, file_path=None, content=None):
        """Calculate file size for either vault file or text content.
    ...
    def _upload_file(self, param):  # noqa: C901
        ...
        parent_message_ts = param.get("parent_message_ts")
        user_filename = param.get("filename")
        filetype = param.get("filetype")

        # Handle destination format conversions
        if destination.startswith("@"):
            # For @username, convert to user ID and open DM channel
            username = destination[1:]
            self.save_progress(f"Converting username {username} to user ID")

            # Use users.list to find the user ID (with pagination support)
            list_params = {}
            ...
            while not found:
                ...
                for member in members:
                    if member.get("name") == username:
                        destination = member.get("id")
                        self.save_progress(f"Found user ID: {destination}")
                        found = True
                        break

                # Check if there are more pages
                response_metadata = users_resp.get("response_metadata", {})
            ...
            self.save_progress(f"Opening DM conversation with user: {destination}")
            open_params = {"users": destination}
            ret_val, open_resp = self._make_slack_rest_call(action_result, SLACK_OPEN_CONVERSATION, open_params)
            if ret_val:
                channel = open_resp.get("channel", {})
                destination = channel.get("id")
                self.save_progress(f"Opened DM channel: {destination}")
            else:
                return action_result.set_status(
                    phantom.APP_ERROR,
            ...
            self.save_progress(f"Converting channel name '{channel_name}' to channel ID")
            self.save_progress("Note: For faster uploads in large workspaces, use channel ID directly (e.g., C1234567890)")
            ...
            if not hasattr(self, '_team_id') or not self._team_id:
                ...
                if ret_val and auth_resp:
                    ...
                    self.save_progress(f"Got team_id: {team_id}")
            else:
            ...
            try:
                while not found:
                    ...
                    ret_val, channels_resp = self._make_slack_rest_call(action_result, SLACK_LIST_CHANNEL, list_params)
                    if not ret_val:
                    ...
                    for channel in channels:
                        if channel.get("name") == channel_name:
                            destination = channel.get("id")
                            self.save_progress(f"Found channel ID: {destination}")
                            found = True
                            break

                    # Check if there are more pages
                    response_metadata = channels_resp.get("response_metadata", {})
                    cursor = response_metadata.get("next_cursor")
        ...
        file_name = None
        file_size = 0
        use_filetype = False  # Only use filetype for text content, not vault files

        # Determine upload mode: vault file or content string
        if "file" in param:
            ...
            try:
                ...
                if not success or not vault_meta_info:
                    ...
                    return action_result.set_status(
                        phantom.APP_ERROR, "{}.{}".format(SLACK_ERROR_UNABLE_TO_FETCH_FILE.format(key="info"), error_msg)
                    )
            except Exception as e:
            ...
            # Calculate file size
            file_size = self._get_file_size(file_path=file_path)

            # Don't use filetype for actual files - let Slack detect it

        elif "content" in param:
            # Upload text content as file
            file_content = param.get("content")

            # Default filetype to "text" if not provided
            if not filetype:
                filetype = "text"

            # Use user-provided filename, or default to "{filetype} snippet"
            file_name = user_filename if user_filename else f"{filetype} snippet"
            file_size = self._get_file_size(content=file_content)

            # Enable filetype for text content
            use_filetype = True
        ...
        self.save_progress(f"Step 1: Getting upload URL for file '{file_name}' (size: {file_size} bytes)")
        get_url_params = {
            "filename": file_name,
            "length": file_size
        }
        ...
        if use_filetype and filetype:
            ...
            self.save_progress(f"Using filetype: {filetype}")

        ret_val, url_resp = self._make_slack_rest_call(action_result, SLACK_GET_UPLOAD_URL, get_url_params)
        if not ret_val:
            message = action_result.get_message()
            if message:
                error_message = f"{SLACK_ERROR_GETTING_UPLOAD_URL}: {message}"
            else:
                error_message = SLACK_ERROR_GETTING_UPLOAD_URL
            return action_result.set_status(phantom.APP_ERROR, error_message)
        ...
        self.save_progress(f"Step 1 complete: Received upload URL and file_id: {file_id}")

        # STEP 2: Upload file content to the provided URL
        self.save_progress("Step 2: Uploading file content to external URL")
        self.save_progress(f"Upload URL: {upload_url}")
        try:
            if file_path:
                # Upload binary file from vault
                with open(file_path, "rb") as f:
                    file_data = f.read()
                    self.save_progress(f"Read {len(file_data)} bytes from file: {file_path}")
                    self.save_progress(f"Filename for upload: {file_name}")

                    # Upload using POST with multipart form data
                    # Form field MUST be named "filename" per Slack API docs
                    self.save_progress("Uploading via multipart form with field 'filename'")
                ...
                self.save_progress(f"Encoded {len(file_data)} bytes of text content: '{file_content}'")
                self.save_progress(f"Filename for upload: {file_name}")
            ...
            self.save_progress(f"Upload response status: {upload_response.status_code}")
            self.save_progress(f"Upload response headers: {dict(upload_response.headers)}")
            self.save_progress(f"Upload response body: {upload_response.text[:500]}")

            if upload_response.status_code not in [200, 201]:
                error_msg = f"{SLACK_ERROR_UPLOADING_TO_URL}: HTTP {upload_response.status_code}"
                if upload_response.text:
                    error_msg += f", Response: {upload_response.text[:200]}"
                self.save_progress(error_msg)
                return action_result.set_status(phantom.APP_ERROR, error_msg)

        except Exception as e:
            error_msg = f"{SLACK_ERROR_UPLOADING_TO_URL}: {self._get_error_message_from_exception(e)}"
            self.save_progress(error_msg)
            return action_result.set_status(phantom.APP_ERROR, error_msg)

        self.save_progress("Step 2 complete: File content uploaded successfully")

        # STEP 3: Complete the upload and share to destination
        self.save_progress("Step 3: Completing upload and sharing")
        self.save_progress(f"Step 3: Using file_id: {file_id}")
        self.save_progress(f"Step 3: Sharing to destination: {destination}")
        ...
        if destination:
            complete_params["channel_id"] = destination

        # Add initial_comment (caption) if provided
        if caption:
            complete_params["initial_comment"] = caption

        # Add thread_ts if replying to a thread
        if parent_message_ts:
            complete_params["thread_ts"] = parent_message_ts

        self.save_progress(f"Step 3 params: files array has {len(files_array)} item(s)")
        self.save_progress(f"Step 3 params JSON: {complete_params}")

        ret_val, complete_resp = self._make_slack_rest_call(action_result, SLACK_COMPLETE_UPLOAD, complete_params)
        if not ret_val:
            message = action_result.get_message()
            # Get full error details from action_result
            action_data = action_result.get_data()
            error_details = ""
            if action_data:
                error_details = f" Full response: {json.dumps(action_data, indent=2)}"
            self.save_progress(f"Step 3 failed: {message}{error_details}")
            if message:
                error_message = f"{SLACK_ERROR_COMPLETING_UPLOAD}: {message}{error_details}"
            else:
                error_message = f"{SLACK_ERROR_COMPLETING_UPLOAD}{error_details}"
            return action_result.set_status(phantom.APP_ERROR, error_message)

        self.save_progress("Step 3 complete: Upload finalized and shared")
        self.save_progress(f"Step 3 response: {json.dumps(complete_resp, indent=2)}")

        # Get the completed file info
        files_list = complete_resp.get("files", [])
        if not files_list:
            return action_result.set_status(phantom.APP_ERROR, "No file information returned after upload completion")

        completed_file = files_list[0]
        file_json = completed_file

        # Build response compatible with existing output data paths
        resp_json = {"file": file_json}

        # Add caption if provided
        if caption:
            resp_json["caption"] = caption

        # Add destination information
        if destination:
            resp_json["destinations"] = [destination]

        # Add sender (bot user) if available
        if "user_id" in complete_resp:
            resp_json["sender"] = complete_resp["user_id"]

        # Process thumbnails if present
        thumbnail_dict = {}
        if file_json:
            ...
            try:
                ...
                if self.is_poll_now():
                    ...
                    if container_count == 1234:
                        sh.kill(pid)
                        self.save_progress(f"Container count set to 1234, stopping slack_bot.py at pid {pid}")
    ...
    def _handle_ask_question(self, action_result, param, user):
        ...
        answer_filename = f"{qid}.json"
        answer_path = f"{local_data_state_dir}/{answer_filename}"
        ...
        path_json = {"qid": qid, "asset_id": str(self.get_asset_id()), "confirmation": confirmation}
        ...
        if not ret_val:
            message = action_result.get_message()
            if message:
                error_message = f"{SLACK_ERROR_ASKING_QUESTION}: {message}"
            else:
    ...
    def _poll_for_question_response(self, action_result, resp_json):
        ...
        This method handles the common polling logic for both _ask_question and
        _ask_question_channel actions. It waits for a response file to appear,
        reads it, parses it, and cleans up the file.
